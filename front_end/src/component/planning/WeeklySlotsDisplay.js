...
const WeeklySlotsDisplay = ({current_week}) => {
    ...
    const controlTimeDisplay = function(startTime){
        startTime ? startTime = Number(startTime.slice(0,2)) : startTime = 24;
        const fullDay = Array.from(Array(24), (_, index) => (index >= startTime ? index : null)).slice(startTime);
        return fullDay
    }
    ...
    const slots = controlTimeDisplay(startTime).flatMap((hour) => {
